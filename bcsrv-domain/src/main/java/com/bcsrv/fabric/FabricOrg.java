...
public class FabricOrg {
    ...
    public FabricOrg(Peers peers, Orderers orderers, FabricStore fabricStore, String cryptoConfigPath)
        ...
        for (int i = 0; i < peers.getPeers().size(); i++) {
            addPeerLocation(peers.getPeers().get(i).getPeerName(), peers.getPeers().get(i).getPeerLocation());
            addEventHubLocation(peers.getPeers().get(i).getPeerEventHubName(), peers.getPeers().get(i).getPeerEventHubLocation());
            setCALocation(peers.getPeers().get(i).getCaLocation());
        }
