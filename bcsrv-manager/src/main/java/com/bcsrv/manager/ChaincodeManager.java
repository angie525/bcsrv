...
import com.bcsrv.fabric.FabricOrg;
import com.bcsrv.fabric.FabricStore;
...
public class ChaincodeManager {
    ...
    public ChaincodeManager(FabricConfig fabricConfig)
        ...
        client.setUserContext(fabricOrg.getPeerAdmin()); // 只有节点管理员可以调用链码
    }

    public FabricOrg getFabricOrg() throws NoSuchAlgorithmException, NoSuchProviderException, InvalidKeySpecException, IOException {
    ...
    public Channel getChannel(FabricOrg fabricOrg, HFClient client)
            throws NoSuchAlgorithmException, NoSuchProviderException, InvalidKeySpecException, IOException, CryptoException, InvalidArgumentException, TransactionException {
        ...
        for (int i = 0; i < peers.get().size(); i++) {
            ...
            //channel.addEventHub(
            //        client.newEventHub("peer0.org1.example.com", "grpc://116.196.125.231:9053", peerProperties));
            ...
            /*==============多节点背书策略时增加其他节点配置==============*/
//            File peerCert2 = Paths.get(config.getCryptoConfigPath(), "/peerOrganizations", "org2.example.com", "peers", "peer0.org2.example.com", "tls/server.crt")
//                    .toFile();
//            if (!peerCert2.exists()) {
//                throw new RuntimeException(
//                        String.format("Missing cert file for: %s. Could not find at location: %s", "peer0.org2.example.com", peerCert2.getAbsolutePath()));
//            }
//            Properties peerProperties2 = new Properties();
//            peerProperties2.setProperty("pemFile", peerCert2.getAbsolutePath());
//            // ret.setProperty("trustServerCertificate", "true"); //testing
//            // environment only NOT FOR PRODUCTION!
...
//            // 在grpc的NettyChannelBuilder上设置特定选项
//            peerProperties2.put("grpc.ManagedChannelBuilderOption.maxInboundMessageSize", 9000000);
//            peerProperties2.put("grpc.NettyChannelBuilderOption.keepAliveTime", new Object[] {5L, TimeUnit.MINUTES});
//            peerProperties2.put("grpc.NettyChannelBuilderOption.keepAliveTimeout", new Object[] {8L, TimeUnit.SECONDS});
//            Peer peer = client.newPeer("peer0.org2.example.com", "grpc://116.196.125.231:9051", peerProperties2);
//
//            channel.addPeer(peer);
//            channel.addEventHub(
//                    client.newEventHub("peer0.org2.example.com", "grpc://116.196.125.231:9053", peerProperties2));
//            if (peers.get().get(i).isAddEventHub()) {
//                channel.addEventHub(
//                        client.newEventHub("peer0.org2.example.com", "grpc://116.196.125.231:7053", peerProperties2));
//            }
            /*==============多节点背书策略时增加其他节点配置==============*/
...
//                        log.debug("event.getEvent().getChaincodeEvent().getPayload().toStringUtf8() = " + event.getEvent().getChaincodeEvent().getPayload().toStringUtf8());//v 1.0.0
    ...
    public Map<String, String> invoke(String fcn, String[] args)
        ...
        System.out.println("===start sendTransactionProposal= " + start + "===");
        ...
        for (ProposalResponse response : transactionPropResp) {
            if (response.getStatus() == ProposalResponse.Status.SUCCESS) {
                ...
                System.out.println("执行失败，原因：" + transactionPropResp.iterator().next().getMessage() +
                        ",fuc=" + fcn + ",args=" + String.valueOf(args));
                failed.add(response);
            }
        }
        ...
        if (failed.size() > 0) {
            ...
            System.out.println("===total time to sendTransactionProposal= " + end + ", " + (end - start) + " ms===");


            //执行交易
            // CompletableFuture<BlockEvent.TransactionEvent> future = channel.sendTransaction(successful);
            start = System.currentTimeMillis();
            System.out.println("===start sendTransaction = " + start + "===");
            channel.sendTransaction(successful).thenApply(transactionEvent -> {
                System.out.println("transactionEvent.isValid():" + transactionEvent.isValid());
                System.out.println("thenApply");
                return null;
            }).get(100, TimeUnit.SECONDS);
            end = System.currentTimeMillis();
            System.out.println("===total time to sendTransaction= " + end + ", " + (end - start) + " ms===");

//                  channel.sendTransaction(successful).exceptionally(e -> {
//                      System.out.println("exceptionally");
//                    if(e instanceof  TransactionEventException){
//                        System.out.println("trans faile ");
//                    }
...
//   //                       .get(chaincode.getInvokeWatiTime(),TimeUnit.SECONDS);
     ...
     * @throws NoSuchAlgorithmException
     */
    public Map<String, String> queryHistoryTransactionByKey(String fcn, String[] args) throws InvalidArgumentException, ProposalException{
        Map<String, String> resultMap = new HashMap<>();
        String payload="";
        QueryByChaincodeRequest queryByChaincodeRequest = client.newQueryProposalRequest();
        queryByChaincodeRequest.setArgs(args);
